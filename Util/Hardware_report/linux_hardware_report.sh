#!/bin/bash
# Linux System Health Report (Blue Professional, final + new sections)
...
color_status_html() {
  local s="$1"
  if echo "$s" | grep -iq 'pass\|ok'; then printf '<b style="color:#1b8f1b;">PASS</b>'
  elif echo "$s" | grep -iq 'fail'; then printf '<b style="color:#c23b22;">FAILED</b>'
  else printf '<b style="color:#f0ad4e;">%s</b>' "$(safe_html "$s")"
  fi
}

color_service_html() {
  local s="$1"
  if [[ "$s" == "RUNNING" ]]; then printf '<b style="color:#1b8f1b">RUNNING</b>'
  else printf '<b style="color:#c23b22">STOPPED</b>'
  fi
}
...
# Failed services and critical logs
...
# SMART Status, Temperatures, Application Services, Top Processes, Quick Metrics
# (Omitted here for brevity — reuse your original blocks as in previous scripts)
# ...

# --- New Panels at Bottom of Left Column ---
cat >> "$TMPHTML" <<HTML
<div class="panel">
...
<div class="pre">$(iostat -xz 1 1 2>/dev/null)</div>
</div>
...
# --- Right-side panels (side section) ---
cat >> "$TMPHTML" <<HTML
</div>
<div>
<div class="panel">
